...
export class PocDashBoardService {
  ...
  public getVolumeChart(params: Gdata): Observable<any> {
    return this.http.post(
      `/v1/fxplt/sys/dashboard/transaction/statistics`,
      params
    );
  }
  ...
  public getRateChart(params: Rdata): Observable<any> {
    return this.http.post(
      `/v1/fxplt/sys/dashboard/history/exchange/rate/dynamics/searches`,
      params
    );
  }
  ...
  public getBankInfo(): Observable<any> {
    return this.http.post(
      `/v1/fxplt/sys/dashboard/centralBank/information/statistics/searches`,
      {}
    );
  }

  public getMapList(): Observable<any> {
    return this.http.post(`/v1/fxplt/sys/dashboard/map/searches`, {});
  }

  public getSpList(pageIndex: number, pageSize: number): Observable<any> {
    const data: any = {
      pageSize: pageSize,
      pageNum: pageIndex
    };
    return this.https
      .post('/v1/fxplt/sys/dashboard/splist/searches', data)
      .pipe(
        map((response: any) => {
          return response;
        })
