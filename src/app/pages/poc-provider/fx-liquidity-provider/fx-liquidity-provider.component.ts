/*
 * @Author: chenyuting
 * @Date: 2025-02-13 13:29:18
 * @LastEditors: chenyuting
 * @LastEditTime: 2025-02-13 14:17:16
 * @Description:
 */
import {
  Component,
...
import { FxLiquidityProviderService } from '@app/core/services/http/poc-provider/fx-liquidity-provider/fx-liquidity-provider.service';
...
@Component({
  selector: 'app-fx-liquidity-provider',
  templateUrl: './fx-liquidity-provider.component.html',
  styleUrls: ['./fx-liquidity-provider.component.less']
})
export class FxLiquidityProviderComponent implements OnInit, AfterViewInit {
  ...
  @ViewChild('foreignTpl', { static: true })
  foreignTpl!: TemplateRef<NzSafeAny>;
  ...
  currencyList: any = [];
  constructor(
    private fxLiquidityProviderService: FxLiquidityProviderService,
  ...
  initSelect() {
    this.commonService
      .getSelect({
        ...
        csePCode: 'FXPLT_EXCHANGE_RATE_VAILD'
      })
      .subscribe((res) => {
    ...
    this.commonService
      .getSelect({ dropDownTypeCode: 'drop_down_currency_status_info' })
      .subscribe((res) => {
  ...
  getDataList(e?: NzTableQueryParams): void {
    ...
    this.currencyList.map((item: any) => {
      if (this.searchParam.currency === item.currencyKey) {
        ...
        (this.listParam.platform = ''), (this.listParam.currency = '');
      }
    });
    ...
    this.fxLiquidityProviderService
      ...
      .subscribe((_: any) => {
        ...
        this.dataList.map((item: any) => {
          item.outCapitalPoolActivation = item.outCapitalPoolActivation
            ? item.outCapitalPoolActivation.toString().split(',')
            : '';
          item.outActivationRate = item.outActivationRate
            ? item.outActivationRate.toString().replaceAll('->', '/').split(',')
            : '';
        });
  ...
  private initTable(): void {
    this.tableConfig = {
      headers: [
        ...
        {
          title: ' Supported Liquidity Tokens',
