 ...
 * @LastEditTime: 2025-02-14 11:02:34
 * @Description:
 */
import {
  Component,
...
export class FxTransactionsComponent implements OnInit, AfterViewInit {
  ...
  ngAfterViewInit(): void {
    this.pageHeaderInfo = {
      title: ``,
      breadcrumb: ['FX Management', 'FX Transactions'],
  ...
  initSelect() {
    this.commonService
      .getSelect({ dropDownTypeCode: 'drop_down_exchange_rate_info' })
      .subscribe((res) => {
    ...
    this.commonService
      .getSelect({
        dropDownTypeCode: 'drop_down_sp_bank_info',
        csePCode: 'FXPLT_SP_BANK_VAILD'
      })
      .subscribe((res) => {
  ...
  getDataList(e?: NzTableQueryParams): void {
    ...
    this.spIdList.map((item: any) => {
      if (this.searchParam.spId === item.spKey) {
        ...
        (this.listParam.spCode = ''), (this.listParam.spName = '');
      }
    });
